...
function updateCode(){
  ...
  if(window.sceneReady){wrapText($('#code-lines'));textGroups[0].els=$$('#workspace .swap-ink').filter(el=>!el.closest('#results,#assistant-result,#running,#deployment-state'));}
}
updateCode();
...
const textGroups=[
  {els:$$('#workspace .swap-ink').filter(el=>!el.closest('#results,#assistant-result,#running,#deployment-state')),at:4.05},
  {els:$$('#results .swap-ink,#assistant-result .swap-ink'),at:6.25},
  {els:[$('#idea')],at:2.017},
  {els:$$('#deployment-state .swap-ink'),at:7.25},
];
function revealTexts(t){
  textGroups.forEach(({els,at},group)=>{
    const progress=(group===1&&(manualRun||manualBacktestComplete)||group===3&&(manualDeploy||manualDeployed))?1:Math.max(0,Math.min(1,(t-at)/.267));
...
let manualTab=null, manualRun=false, manualBacktestComplete=false, manualDeploy=false, manualDeployed=false, ideaText=DEFAULT_PROMPT, scrubMode=false;
const runClock={progress:0}, deployClock={progress:0};
const runTimeline=gsap.timeline({paused:true,onUpdate:()=>sync(tl.time()),onComplete:()=>{manualRun=false;manualBacktestComplete=true;manualTab='backtest';sync(tl.time());}})
 .to(runClock,{progress:1,duration:1.1,ease:'none'});
const deployTimeline=gsap.timeline({paused:true,onUpdate:()=>sync(tl.time()),onComplete:()=>{manualDeploy=false;manualDeployed=true;sync(tl.time());}})
 .to(deployClock,{progress:1,duration:.55,ease:'none'});
...
function sync(t){
  ...
  const ready=manualBacktestComplete||manualDeploy||manualDeployed||(!manualRun&&t>=6.25);
  const backtestProgress=manualRun?runClock.progress:clamp((t-5.83)/.42);
  const tab=manualRun||manualDeploy||manualDeployed?'backtest':manualTab||(t>=5.83?'backtest':'code');
  showTab(tab);
  ...
  const reveal=manualBacktestComplete||manualDeploy||manualDeployed?1:clamp((t-6.25)/.50);
  $('#equity-line').style.strokeDasharray=`${curveLength}`;$('#equity-line').style.strokeDashoffset=`${curveLength*(1-reveal)}`;
  const deploying=manualDeploy||(!manualRun&&!manualBacktestComplete&&!manualDeployed&&t>=7.25&&t<7.65);
  const deployed=manualDeployed||(!manualRun&&!manualBacktestComplete&&t>=7.65);
  const deploymentProgress=manualDeploy?deployClock.progress:clamp((t-7.25)/.4);
  label('#run',deployed?'Manage':deploying?'Deploying…':ready?'Deploy':!generated?'Generating…':backtestProgress>0?'Backtesting…':'Run backtest');
  $('#run').disabled=deploying||(!ready&&(!generated||manualRun||t>=5.83));
  $('#run').dataset.action=deployed?'manage':ready?'deploy':'backtest';
  $('#rerun').disabled=manualRun||deploying||!generated;
  label('.status-badge',deployed?'Deployed':deploying?'Deploying':'Draft');
  $('.status-badge').classList.toggle('deployed',deployed);
  $('#deployment-state').hidden=!(deploying||deployed);
  label('#deployment-label',deployed?'✓ Strategy deployed':'Deploying strategy…');
  $('#deployment-state progress').hidden=deployed;$('#deployment-state progress').value=deploymentProgress;
...
function resetManual(){runTimeline.pause();deployTimeline.pause();manualTab=null;manualRun=false;manualBacktestComplete=false;manualDeploy=false;manualDeployed=false;runClock.progress=0;deployClock.progress=0;$('#sheet').hidden=true;}
...
function run(){if(manualRun||manualDeploy)return;tl.pause();$('#flow').pause();$('#sheet').hidden=true;manualRun=true;manualBacktestComplete=false;manualDeployed=false;manualTab='backtest';runClock.progress=0;sync(tl.time());runTimeline.restart();}
function deploy(){if(manualDeploy||$('#results').hidden)return;tl.pause();$('#flow').pause();manualDeploy=true;manualTab='backtest';deployClock.progress=0;sync(tl.time());deployTimeline.restart();}
$('#run').onclick=()=>{const action=$('#run').dataset.action;if(action==='deploy')deploy();else if(action==='manage')sheet('Manage strategy','<p>Semiconductor Trend · Deployed</p><p>Daily evaluation · Long only · 4 assets</p><p>Local product demonstration. No live trading account is connected.</p>');else run();};
$('#rerun').onclick=run;
