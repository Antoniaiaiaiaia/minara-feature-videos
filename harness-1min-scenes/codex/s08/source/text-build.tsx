...
export interface TextBuildProps {
  ...
  pushDuration?: number;
  /** Still frames between settled phrases. */
  holdDuration?: number;
...
export function measureWordSizes(
  ...
  ctx.letterSpacing = `${-0.03 * fontSize}px`;
...
export function TextBuild({
  ...
  const words = useMemo(() => text.includes("\n") ? text.split("\n") : text.split(" "), [text]);
  ...
  const sizes = useMemo(
    () =>
      measured
        ? measureWordSizes(words, axis, fontSize, fontWeight, face)
        : fallbackWordSizes(words, axis, fontSize),
    [measured, words, axis, fontSize, fontWeight, face],
  );
  ...
  return (
    ...
    >
      ...
      >
        {words.map((word, j) => {
          const entryStart =
            j === 0 ? 0 : firstDuration + j * holdDuration + (j - 1) * pushDuration;
          const entryEnd =
          ...
          const primaryFrom =
            j === 0
              ...
              : axis === "y"
                ? target - offset
                : target + Math.max(offset, (sizes[j] + spacing) / 2);
          ...
          if (frame < entryStart) {
            ...
            for (let w = j + 1; w < n; w++) {
              const pushStart = firstDuration + w * holdDuration + (w - 1) * pushDuration;
