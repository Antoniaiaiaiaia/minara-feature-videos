...
const output=path.resolve(here,'../../output/s04-s05-portfolio-orders-v4.mp4');
...
execFileSync('ffmpeg',['-n','-hide_banner','-loglevel','error','-framerate',String(fps),'-i',path.join(frames,'f-%05d.png'),'-i',path.join(here,'signal.wav'),'-filter_complex','[1:a]adelay=5020,apad,atrim=duration=12.5[a]','-map','0:v','-map','[a]','-frames:v',String(count),'-t','12.5','-c:v','libx264','-crf','13','-pix_fmt','yuv420p','-colorspace','bt709','-color_primaries','bt709','-color_trc','bt709','-c:a','aac','-b:a','192k','-movflags','+faststart',output],{stdio:'inherit'});
...
assert.equal(probe.streams.length,2);assert.equal(probe.streams.find(s=>s.codec_type==='audio').codec_name,'aac');assert(Math.abs(+probe.format.duration-count/fps)<.001);
await writeFile(path.join(verification,'render.json'),JSON.stringify({output,frames:count,fps,errors,probe},null,2));
execFileSync('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',output,'-vf',"select='eq(n,12)+eq(n,43)+eq(n,113)+eq(n,168)+eq(n,203)+eq(n,214)+eq(n,235)+eq(n,265)+eq(n,295)+eq(n,325)+eq(n,355)+eq(n,365)',scale=480:270,tile=4x3",'-frames:v','1',path.join(verification,'contact-sheet.jpg')],{stdio:'inherit'});
execFileSync('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',output,'-vf',"select='eq(n,214)',scale=1600:900",'-frames:v','1',path.join(verification,'order-check.png')],{stdio:'inherit'});
