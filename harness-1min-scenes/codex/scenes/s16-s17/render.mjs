...
const output = path.resolve(here, '../../output2/s16-s17-learns-from-your-trades.mp4');
const fps = 30;
const count = 231;
const duration = count / fps;
const errors = [];
const playwrightPath = pathToFileURL(path.resolve(
...
try {
  ...
  await page.waitForFunction(() => (
    window.scene && window.scene.duration === 7.7 &&
    window.S16Title?.ready === true &&
  ...
  for (let frame = 0; frame < count; frame++) {
    await page.evaluate(async seconds => {
      ...
      await window.S16Title.seek(Math.round(seconds * 30));
...
const frameExpr = [18, 48, 71, 90, 127, 165, 185, 207, 220, 230].map(frame => `eq(n\\,${frame})`).join('+');
execFileSync('ffmpeg', [
