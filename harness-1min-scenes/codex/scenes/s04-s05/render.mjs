...
const output=path.resolve(here,'../../output/s04-s05-every-asset-one-intelligence.mp4');
...
execFileSync('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',output,'-vf',"select='eq(n,12)+eq(n,32)+eq(n,44)+eq(n,53)+eq(n,76)+eq(n,108)+eq(n,119)+eq(n,126)+eq(n,145)+eq(n,166)+eq(n,200)+eq(n,232)',scale=480:270,tile=4x3",'-frames:v','1',path.join(verification,'contact-sheet.jpg')],{stdio:'inherit'});
execFileSync('ffmpeg',['-y','-hide_banner','-loglevel','error','-i',output,'-vf',"select='eq(n,221)'",'-frames:v','1',path.join(verification,'final-signals.png')],{stdio:'inherit'});
