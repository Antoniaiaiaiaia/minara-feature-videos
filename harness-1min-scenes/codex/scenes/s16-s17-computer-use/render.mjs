...
const output = path.resolve(here, '../../output2/s16-s17-computer-use.mp4');
const fps = 30, count = 723, duration = count / fps, errors = [];
...
try {
  ...
  await page.waitForFunction(() => (
    window.scene?.ready && window.scene.duration === 24.1 &&
    window.BeyondTitle?.ready === true && window.ComputerTitle?.ready === true && document.fonts.status === 'loaded' &&
  ...
  for (let frame = 0; frame < count; frame++) {
    await page.evaluate(async seconds => {
      ...
      window.ComputerTitle.seek(Math.round((seconds - window.scene.cues.grid) * 30));
      const video = document.querySelector('#flow');
      const backgroundTime = seconds % video.duration;
      if (video.seeking) await new Promise(resolve => video.addEventListener('seeked', resolve, {once: true}));
      if (Math.abs(video.currentTime - backgroundTime) > 1e-5) {
        await new Promise(resolve => {
          video.addEventListener('seeked', resolve, {once: true});
          video.currentTime = backgroundTime;
        });
      }
...
const frameExpr = [18, 104, 194, 270, 424, 555, 588, 620, 660, 722].map(frame => `eq(n\\,${frame})`).join('+');
execFileSync('ffmpeg', [
...
execFileSync('ffmpeg', [
  '-y', '-hide_banner', '-loglevel', 'error', '-i', output,
  '-vf', "select='eq(n\\,722)',scale=1600:-2", '-frames:v', '1',
  path.join(verification, 'final-frame-722.png'),
], {stdio: 'inherit'});
