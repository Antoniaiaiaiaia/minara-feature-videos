...
const cards=assets.map((a,i)=>{
 ...
 wrap.innerHTML=`<button class="market-card" aria-label="Explore ${a.symbol}">${marketHTML(a)}<div class="wall-order"><div class="wall-order-summary"><span>Market buy</span><b>$${amount}</b></div><div class="wall-buy">Buy ${a.symbol}</div><div class="wall-cursor"><svg viewBox="0 0 32 40"><path d="M3 2 27 24l-11 1 6 11-6 3-6-12-7 8Z"/></svg><i></i></div><div class="wall-filled"><i><svg viewBox="0 0 24 24"><path d="m5 12 4 4 10-10"/></svg></i><div><b>Order placed</b><small>${a.symbol} · $${amount}</small></div></div></div></button>`;$('#wall').append(wrap);
 const cue=5.02+((i*11)%19)*.1;
 const c={wrap,card:wrap.firstElementChild,tag:wrap.querySelector('.signal-overlay'),metrics:wrap.querySelector('.metrics'),order:wrap.querySelector('.wall-order'),buy:wrap.querySelector('.wall-buy'),cursor:wrap.querySelector('.wall-cursor'),filled:wrap.querySelector('.wall-filled'),col:i%7,row:Math.floor(i/7),signal:i%5!==4,trade:i%5<3,cue,orderCue:cue+.6+((i*3)%5)*.12,clickedAt:null};
 c.card.onclick=()=>{if(clock.t<4.5)return;c.clickedAt=clock.t-.32;play();};
 return c;
});
function paintWallOrder(c,t,on){
 const start=c.clickedAt??c.orderCue,active=on&&(c.trade||c.clickedAt!==null),phase=t-start;
 const reveal=active?beat(t,start,.18,'enter'):0,filled=active?clamp(beat(phase,.48,.2,'back.out(1.8)')):0;
 const signal=on&&c.signal?clamp(beat(t,c.cue,.24,'back.out(1.5)')):0;
 c.tag.style.opacity=signal*(1-reveal);c.tag.style.transform=`scale(${.94+.06*signal})`;
 c.metrics.style.opacity=1-reveal;c.order.style.opacity=reveal;c.order.style.transform=`translateY(${8*(1-reveal)}px)`;
 c.buy.style.transform=`scale(${1-.06*beat(phase,.32,.055)+.06*beat(phase,.375,.1)})`;
 c.filled.style.opacity=filled;c.filled.style.transform=`scale(${.9+.1*beat(phase,.48,.2,'back.out(1.8)')})`;
 c.cursor.style.opacity=active&&phase>.15&&phase<.48?1:0;c.cursor.style.transform=`translate(${16*(1-beat(phase,.12,.2,'power2.out'))}px,${12*(1-beat(phase,.12,.2,'power2.out'))}px)`;
 c.cursor.querySelector('i').style.opacity=phase>=.32&&phase<.46?1:0;
 c.wrap.dataset.order=filled>.99?'filled':reveal>.99?'buy':'idle';c.card.setAttribute('aria-label',filled>.99?`${c.wrap.dataset.symbol} order placed`:`Buy ${c.wrap.dataset.symbol}`);
}
...
function update(t){
 ...
 gsap.set('#wall',{scale:wallScale,opacity:opening?1-retreat:signalWall?1:0,filter:`blur(${signalWall?0:retreat*3}px)`});
 cards.forEach((c,i)=>{
 ...
 gsap.set('#edge',{opacity:opening||signalWall?1:0});
...
function seek(t){cards.forEach(c=>c.clickedAt=null);manual=false;$('#model-menu').hidden=true;timeline.pause();flow.pause();timeline.time(Math.max(0,Math.min(DURATION,t)));update(clock.t);syncBackground(clock.t);$('#play').textContent='Play';}
