...
const DURATION=22.55, ORIGINAL_DURATION=223/30;
const cues={intent:1.833,cut:6.88,order:14.08,review:15.25,confirmation:15.3,confirm:16.7,result:16.95,filled:17.95,done:18.2,positionClick:19.35,position:19.6};
...
tl.set('#composer',{y:0,height:130,scale:1},cues.order);tl.set('.answer',{autoAlpha:1},12.65);
surface('#sent',14.08,.22);stream(sentWords,14.18,60);
grow('.agent-name',14.18,.22);grow('.agent-name > .icon',14.25,.18);
stream(answerWords,14.25,55);
surface('.order-inline',12.7,.38);
gsap.set('.order-inline',{x:168,y:-55});tl.to('.order-inline',{x:0,y:0,duration:.72,ease:'power3.inOut'},13.48);
grow('.order-inline h3',12.86,.22);
grow('.order-inline .kv',12.98,.2,.07);grow('.order-inline .kv > *',13.01,.18,.035);
grow('#review',13.2,.22);
tl.fromTo('#composer',{autoAlpha:0,y:95},{autoAlpha:1,y:0,duration:.34,immediateRender:false},14.6);
tl.fromTo('.composer-footer > *',{autoAlpha:0,y:10},{autoAlpha:1,y:0,duration:.2,stagger:.02,immediateRender:false},14.75);
tl.to('#review',{scale:.95,duration:.07},cues.review);tl.to('#review',{scale:1,duration:.12},15.32);
tl.to('#confirm-layer',{autoAlpha:1,duration:.17},cues.confirmation);
surface('.dialog',15.31,.28);
grow('.dialog h2',15.4,.2);grow('.dialog p',15.45,.2);
grow('.confirm-rows > div',15.49,.22,.045);grow('.confirm-rows > div > *',15.52,.19,.023);
grow('.confirm-actions > button',15.66,.21,.055);
// Confirmation resolves by ~16.25; click at 16.7.
tl.to('#confirm',{scale:.96,duration:.09},cues.confirm);tl.to('#confirm',{scale:1,duration:.14},16.79);
tl.to('#confirm-layer',{autoAlpha:0,duration:.18},16.87);tl.to('#chat',{autoAlpha:0,duration:.16},16.9);
tl.set('#workflow',{autoAlpha:1},cues.result);
grow('#workflow h1',16.97,.28);grow('#workflow .subtitle',17.03,.25);
grow('.run-meta > *',17.06,.24,.035);
surface('.result',17.12,.3);grow('.result h3',17.2,.22);grow('#pending-result',17.28,.22);
surface('.workflow-canvas',17.19,.34);grow('.canvas-tools',17.35,.2);grow('.canvas-tools > *',17.39,.16,.035);
// Upstream card stagger, then 4-frame delay before its content fills.
for(let i=1;i<=4;i++){
  const born=17.27+(i-1)*ref.cardGap;surface(`.node-${i}`,born,.22);
...
tl.set('#filled-result',{autoAlpha:1},cues.filled);stream(filledWords,17.96,60);
grow('.order-submitted',18.14,.22);grow('.order-submitted > *',18.17,.2,.025);
grow('#open-position',18.27,.25);grow('#open-position > .icon',18.4,.18);
tl.to('#open-position',{scale:.96,duration:.08},cues.positionClick);tl.to('#open-position',{scale:1,duration:.13},19.43);
tl.to('#workflow',{autoAlpha:0,duration:.16},19.52);tl.set('#portfolio',{autoAlpha:1},cues.position);
surface('.sidebar',19.6,.31);grow('.nav',19.68,.23,.044);grow('.nav .icon',19.74,.2,.044);
surface('.wallet-select',19.61,.28);planText($('.wallet-select'),19.7);
grow('.wallet-controls > *',19.67,.23,.045);grow('.equity',19.72,.26);grow('.available',19.78,.24);
grow('.portfolio-tabs > *',19.82,.23,.04);grow('.position-tabs',19.87,.2);grow('.position-tabs > *',19.91,.2,.04);
grow('.positions',19.93,.24);grow('.positions th',19.97,.2,.022);grow('#position-row > td',20.09,.22,.028);
grow('.receipt',20.36,.24);grow('.receipt > *',20.39,.2,.035);grow('.view-run',20.53,.23);
addResearchMotion();
// Dynamic labels retain their animated wrapper when their text changes.
planText($('#tab-title'),2.02);planText($('#run-state'),17.17);
...
tl.to('.client',{x:-2050,duration:1,ease:'power3.inOut'},21.55);
tl.to({}, {duration:.01},DURATION-.01);window.__timelines={'s06-s07':tl};
const anchors=[[0,0],[1.833,1.833],[2.2,6.88],[2.55,14.08],[2.72,15.3],[3.8,16.7],[4.0,16.95],[5.7,19.35],[5.93,19.6],[ORIGINAL_DURATION,21.55]];
...
const pointerCues=[{selector:'#send',start:5.875,click:6.491,end:6.88},{selector:'#review',start:14.98,click:15.25,end:15.32},{selector:'#confirm',start:16.39,click:16.7,end:16.95},{selector:'#open-position',start:19.07,click:19.35,end:19.55}];
...
$('#composer').addEventListener('submit',event=>{event.preventDefault();if(!interactive)return;if($('#intent').value.trim()!==prompt){$('#input-error').textContent='This preview supports the preset order. Restore the prompt to continue.';return;}$('#input-error').textContent='';approved=false;seek(at(6.88));play(at(15.24));});
$('#review').onclick=()=>{if(!interactive||step!=='order')return;seek(at(15.3));play(at(16.6));};
$('#cancel').onclick=()=>{if(!interactive||approved)return;seek(at(15.24));$('#review').focus();};
$('#confirm').onclick=()=>{if(!interactive||approved||step!=='confirmation')return;approved=true;seek(at(16.7));play(at(19));};
$('#open-position').onclick=()=>{if(!interactive||!approved||step!=='result')return;seek(at(19.6));play(at(21.3));};
