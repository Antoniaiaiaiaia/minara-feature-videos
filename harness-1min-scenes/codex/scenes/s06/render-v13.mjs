...
const frames=path.join(here,'render-frames-v13'),outputDir=path.resolve(here,'../../output2');
...
const output=path.join(outputDir,`s06-s07-execution-exit-v${Math.max(12,...versions)+1}.mp4`);
...
try{
  ...
  await page.goto(pathToFileURL(path.join(here,'index-v13.html')).href);
  ...
  const exit=await page.evaluate(()=>{
    ...
    const start=rect(17.65),middle=rect(18.15),end=rect(18.65),reset=rect(17.65);
...
await writeFile(path.join(here,'verification-v13','render.json'),JSON.stringify({output,frames:count,fps,errors,probe},null,2));
