...
const DURATION=15.35, ORIGINAL_DURATION=223/30;
const cues={intent:1.833,cut:6.88,review:8.05,confirmation:8.1,confirm:9.5,result:9.75,filled:10.75,done:11,positionClick:12.15,position:12.4};
...
// Original answer-stream cut + glide; surfaces/contents rebuild in sequence.
tl.set('#composer',{y:0,height:130},cues.cut);tl.set('.answer',{autoAlpha:1},cues.cut);
surface('#sent',6.88,.22);stream(sentWords,6.98,60);
grow('.agent-name',6.98,.22);grow('.agent-name > .icon',7.05,.18);
stream(answerWords,7.05,55);
surface('.order-inline',7.21,.25);
grow('.order-inline h3',7.21+ref.fillDelay,.22);
grow('.order-inline .kv',7.39,.2,.06);grow('.order-inline .kv > *',7.42,.18,.028);
grow('#review',7.51,.22);
tl.fromTo('#composer',{autoAlpha:0,y:95},{autoAlpha:1,y:0,duration:.34,immediateRender:false},7.4);
tl.fromTo('.composer-footer > *',{autoAlpha:0,y:10},{autoAlpha:1,y:0,duration:.2,stagger:.02,immediateRender:false},7.55);
tl.to('#review',{scale:.95,duration:.07},cues.review);tl.to('#review',{scale:1,duration:.12},8.12);
tl.to('#confirm-layer',{autoAlpha:1,duration:.17},cues.confirmation);
surface('.dialog',8.11,.28);
grow('.dialog h2',8.2,.2);grow('.dialog p',8.25,.2);
grow('.confirm-rows > div',8.29,.22,.045);grow('.confirm-rows > div > *',8.32,.19,.023);
grow('.confirm-actions > button',8.46,.21,.055);
// All text resolves by ~9.05; click at 5.65. The previous three-second wait is gone.
tl.to('#confirm',{scale:.96,duration:.09},cues.confirm);tl.to('#confirm',{scale:1,duration:.14},9.59);
tl.to('#confirm-layer',{autoAlpha:0,duration:.18},9.67);tl.to('#chat',{autoAlpha:0,duration:.16},9.7);
tl.set('#workflow',{autoAlpha:1},cues.result);
grow('#workflow h1',9.77,.28);grow('#workflow .subtitle',9.83,.25);
grow('.run-meta > *',9.86,.24,.035);
surface('.result',9.92,.3);grow('.result h3',10,.22);grow('#pending-result',10.08,.22);
surface('.workflow-canvas',9.99,.34);grow('.canvas-tools',10.15,.2);grow('.canvas-tools > *',10.19,.16,.035);
// Upstream: 7.35-frame card stagger, then 4-frame delay before its content fills.
for(let i=1;i<=4;i++){
  const born=10.07+(i-1)*ref.cardGap;surface(`.node-${i}`,born,.22);
...
tl.set('#filled-result',{autoAlpha:1},cues.filled);stream(filledWords,10.76,60);
grow('.order-submitted',10.94,.22);grow('.order-submitted > *',10.97,.2,.025);
grow('#open-position',11.07,.25);grow('#open-position > .icon',11.2,.18);
tl.to('#open-position',{scale:.96,duration:.08},cues.positionClick);tl.to('#open-position',{scale:1,duration:.13},12.23);
tl.to('#workflow',{autoAlpha:0,duration:.16},12.32);tl.set('#portfolio',{autoAlpha:1},cues.position);
surface('.sidebar',12.4,.31);grow('.nav',12.48,.23,.044);grow('.nav .icon',12.54,.2,.044);
surface('.wallet-select',12.41,.28);planText($('.wallet-select'),12.5);
grow('.wallet-controls > *',12.47,.23,.045);grow('.equity',12.52,.26);grow('.available',12.58,.24);
grow('.portfolio-tabs > *',12.62,.23,.04);grow('.position-tabs',12.67,.2);grow('.position-tabs > *',12.71,.2,.04);
grow('.positions',12.73,.24);grow('.positions th',12.77,.2,.022);grow('#position-row > td',12.89,.22,.028);
grow('.receipt',13.16,.24);grow('.receipt > *',13.19,.2,.035);grow('.view-run',13.33,.23);
// Dynamic labels retain their animated wrapper when their text changes.
planText($('#tab-title'),2.02);planText($('#run-state'),9.97);
...
tl.to('.client',{x:-2050,duration:1,ease:'power3.inOut'},14.35);
tl.to({}, {duration:.01},DURATION-.01);window.__timelines={'s06-s07':tl};
const anchors=[[0,0],[1.833,1.833],[2.2,6.88],[2.72,8.1],[3.8,9.5],[4.0,9.75],[5.7,12.15],[5.93,12.4],[ORIGINAL_DURATION,14.35]];
...
const pointerCues=[{selector:'#send',start:5.875,click:6.491,end:6.88},{selector:'#review',start:7.78,click:8.05,end:8.12},{selector:'#confirm',start:9.19,click:9.5,end:9.75},{selector:'#open-position',start:11.87,click:12.15,end:12.35}];
...
function cameraAt(t){
  ...
  else if(t<8.1){const scale=snapcnMotion.shotBScale(t,6.97,.64,ref.pullbackFrom,ref.undershoot,.26),prev=snapcnMotion.shotBScale(t-1/30,6.97,.64,ref.pullbackFrom,ref.undershoot,.26);const blur=Math.min(3,Math.abs(scale-prev)*(1080/2-ref.focusY*1080)/4);gsap.set('#camera',{scale,x:0,y:-27*(1-gsap.parseEase('power2.out')(clamp01((t-cues.cut)/.5))),transformOrigin:`50% ${ref.focusY*100}%`,filter:blur>.15?`blur(${blur}px)`:'none'});}
...
function sync(t){const c=mode==='original'?mapTime(t):t;tl.pause(c);titleBuildAt(c);gsap.set(tickerTrack,{x:-(Math.max(0,c-2.9)*34%tickerGroup.offsetWidth)});cameraAt(c);setLabel('#tab-title',c>=cues.position?'Portfolio':c>=cues.result?'BTC market order':'Chat');$('#tab-icon').setAttribute('href',c>=cues.position?'#i-wallet':c>=cues.result?'#i-grid':'#i-chat');setLabel('#run-state',c>=cues.done?'✓ Done':'Running');$('#run-state').classList.toggle('done',c>=cues.done);if(!interactive&&c>=cues.cut)$('#intent').value='';if(interactive||c>=cues.cut)$('#typing-caret').hidden=true;$('#send-bloom').hidden=c>=cues.cut;step=c<cues.intent?'title':c<cues.cut?'intent':c<cues.confirmation?'order':c<cues.result?'confirmation':c<cues.position?'result':'position';pointerAt(c);if(Number.isFinite(flow.duration)&&!flow.seeking&&Math.abs(flow.currentTime-t)>.06)flow.currentTime=t%flow.duration;window.parent.postMessage({type:'s0607-status',time:t,duration:duration(),step,playing:!!driver?.isActive(),interactive},'*');}
...
function at(t){return mode==='original'?mapTime(t,true):t;}
function interactiveStart(){pause();interactive=true;approved=false;seek(at(5.8));$('#intent').value=prompt;$('#intent').focus();}
$('#composer').addEventListener('submit',event=>{event.preventDefault();if(!interactive)return;if($('#intent').value.trim()!==prompt){$('#input-error').textContent='This preview supports the preset order. Restore the prompt to continue.';return;}$('#input-error').textContent='';approved=false;seek(at(6.88));play(at(8.04));});
$('#review').onclick=()=>{if(!interactive||step!=='order')return;seek(at(8.1));play(at(9.4));};
$('#cancel').onclick=()=>{if(!interactive||approved)return;seek(at(8.04));$('#review').focus();};
$('#confirm').onclick=()=>{if(!interactive||approved||step!=='confirmation')return;approved=true;seek(at(9.5));play(at(11.8));};
$('#open-position').onclick=()=>{if(!interactive||!approved||step!=='result')return;seek(at(12.4));play(at(14.1));};
$('#view-run').onclick=()=>{if(!interactive||!approved)return;seek(at(11.8));};
