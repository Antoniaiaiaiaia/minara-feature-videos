...
gsap.set('#camera,#cursor,#confirm-layer,#workflow,#sent,.answer',{autoAlpha:0});
...
// Keep Workflow for the former Portfolio interval. Light travels left to right.
const flowPulses=[1,2,3].map(i=>{
  const pulse=$(`#edge-${i}`).cloneNode();pulse.removeAttribute('id');pulse.setAttribute('class','flow-pulse');
  pulse.style.cssText='stroke:#a8ffcf;stroke-width:3;stroke-linecap:round;filter:drop-shadow(0 0 5px #29ed91);';
  pulse.setAttribute('stroke-dasharray','24 117');
  $('.edges').append(pulse);return pulse;
});
gsap.set(flowPulses,{autoAlpha:0});tl.to(flowPulses,{autoAlpha:1,duration:.35},16.6);
const pan={x:0,y:0,zoom:1},manualPan={x:0,y:0};let manualZoom=1;
const workflowCanvas=$('.workflow-canvas');
function applyPan(){
  const x=pan.x+manualPan.x,y=pan.y+manualPan.y;
  const zoom=pan.zoom*manualZoom;
  gsap.set('.workflow-pan',{x,y,scale:zoom,transformOrigin:'800px 134px'});
  workflowCanvas.style.backgroundSize=`${15*zoom}px ${15*zoom}px`;
  workflowCanvas.style.backgroundPosition=`${x+800*(1-zoom)}px ${y+134*(1-zoom)}px`;
}
tl.to('#zoom-in',{scale:.86,duration:.07},17.4);tl.to('#zoom-in',{scale:1,duration:.16},17.47);
tl.to(pan,{zoom:1.22,duration:.36,ease:'power2.inOut'},17.4);
tl.to(pan,{x:-115,y:-28,duration:.67,ease:'power2.inOut'},18.05);
tl.to(pan,{x:-32,y:16,duration:.36,ease:'power2.inOut'},18.82);
...
for(const [selector,at] of [['#tab-title .swap-text',cues.result]])tl.fromTo(selector,{autoAlpha:0,scale:textMotion.scale,filter:`blur(${textMotion.blur}px)`},{autoAlpha:1,scale:1,filter:'blur(0px)',duration:textMotion.duration,ease:textMotion.ease,immediateRender:false},at);
...
const pointerCues=[{selector:'#send',start:5.875,click:6.491,end:6.88},{selector:'#review',start:13.38,click:13.65,end:13.72},{selector:'#confirm',start:14.79,click:15.1,end:15.35},{selector:'#zoom-in',start:16.95,click:17.4,end:17.55}];
...
function pointerAt(t){
  if(!interactive&&t>=17.55&&t<19.65){
    const stage=$('#stage').getBoundingClientRect(),canvas=workflowCanvas.getBoundingClientRect(),button=$('#zoom-in').getBoundingClientRect(),unit=1920/stage.width;
    const approach=gsap.parseEase('minaraEnter')(clamp01((t-17.55)/.5)),leave=clamp01((t-19.25)/.4);
    const startX=(button.x+button.width/2-stage.x)*unit,startY=(button.y+button.height/2-stage.y)*unit;
    const targetX=(canvas.x-stage.x)*unit+800+20*pan.zoom+pan.x;
    const targetY=(canvas.y-stage.y)*unit+134+76*pan.zoom+pan.y;
    gsap.set('#cursor',{x:startX+(targetX-startX)*approach-3+leave*55,y:startY+(targetY-startY)*approach-2+leave*30,autoAlpha:1-leave});
    gsap.set('#ring',{autoAlpha:t>=18&&t<19.24?.6:0,scale:.72});return;
  }
  const cue=pointerCues.find(c=>t>=c.start&&t<c.end);if(!cue||interactive){gsap.set('#cursor',{autoAlpha:0});return;}const stage=$('#stage').getBoundingClientRect(),target=$(cue.selector).getBoundingClientRect(),scale=1920/stage.width;const x=(target.x+target.width/2-stage.x)*scale-3,y=(target.y+target.height/2-stage.y)*scale-2,progress=gsap.parseEase(cue.selector==='#send'?'minaraEnter':'minaraSnap')(clamp01((t-cue.start)/(cue.selector==='#send'?.42:cue.click-cue.start)));gsap.set('#cursor',{autoAlpha:1,x:x+(1-progress)*(cue.selector==='#send'?1920*.86-x:85),y:y+(1-progress)*(cue.selector==='#send'?1080*1.02-y:62)});const click=clamp01((t-cue.click)/.25);gsap.set('#ring',{autoAlpha:t>=cue.click?1-click:0,scale:.6+click*1.1});}
function setLabel(selector,text){const e=$(selector+' .swap-text')||$(selector);e.textContent=text;}
function sync(t){const c=t<1.8?t:t+2.3;tl.pause(c);applyPan();titleBuildAt(c);gsap.set(tickerTrack,{x:-(Math.max(0,c-2.9)*34%tickerGroup.offsetWidth)});cameraAt(c);gsap.set('#camera',{autoAlpha:t<1.8?0:1});setLabel('#tab-title',c>=cues.result?'BTC market order':c>=cues.order?'Trading research team':'Chat');$('#tab-icon').setAttribute('href',c>=cues.result?'#i-grid':'#i-chat');setLabel('#run-state','Running');setLabel('#run-duration',`Elapsed: ${Math.max(0,c-cues.result).toFixed(1)}s`);flowPulses.forEach((pulse,i)=>gsap.set(pulse,{strokeDashoffset:-((Math.max(0,c-16.6)*100+i*47)%141)}));if(!interactive&&c>=cues.cut)$('#intent').value='';if(interactive||c>=cues.cut)$('#typing-caret').hidden=true;$('#send-bloom').hidden=c>=cues.cut;step=c<cues.intent?'title':c<cues.cut?'intent':c<cues.order?'research':c<cues.confirmation?'order':c<cues.result?'confirmation':'result';pointerAt(c);if(Number.isFinite(flow.duration)&&!flow.seeking&&Math.abs(flow.currentTime-t)>.06)flow.currentTime=t%flow.duration;window.parent.postMessage({type:'s0607-status',time:t,duration:duration(),step,playing:!!driver?.isActive(),interactive},'*');}
function pause(){driver?.kill();driver=null;sync(playhead.time);}
function seek(t){manualZoom=1;manualPan.x=manualPan.y=0;driver?.kill();driver=null;playhead.time=Math.max(0,Math.min(duration(),Number(t)||0));sync(playhead.time);}
...
for(const [id,factor] of [['#zoom-in',1.2],['#zoom-out',1/1.2],['#zoom-fit',0]])$(id).addEventListener('click',()=>{
  pause();interactive=true;gsap.set('#cursor',{autoAlpha:0});
  if(factor)manualZoom=Math.max(.6,Math.min(2.5,pan.zoom*manualZoom*factor))/pan.zoom;
  else{manualZoom=1/pan.zoom;manualPan.x=-pan.x;manualPan.y=-pan.y;}
  applyPan();
});
let activeDrag=null;
workflowCanvas.addEventListener('pointerdown',e=>{
  if(e.button!==0||e.target.closest('.canvas-tools'))return;
  pause();interactive=true;gsap.set('#cursor',{autoAlpha:0});
  const scale=$('#stage').getBoundingClientRect().width/1920;
  activeDrag={x:e.clientX,y:e.clientY,startX:manualPan.x,startY:manualPan.y,scale};
  workflowCanvas.setPointerCapture(e.pointerId);workflowCanvas.classList.add('dragging');e.preventDefault();
});
workflowCanvas.addEventListener('pointermove',e=>{
  if(!activeDrag)return;
  manualPan.x=activeDrag.startX+(e.clientX-activeDrag.x)/activeDrag.scale;
  manualPan.y=activeDrag.startY+(e.clientY-activeDrag.y)/activeDrag.scale;applyPan();
});
for(const event of ['pointerup','pointercancel','lostpointercapture'])workflowCanvas.addEventListener(event,()=>{activeDrag=null;workflowCanvas.classList.remove('dragging');});
