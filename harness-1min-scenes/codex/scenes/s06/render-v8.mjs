...
const frames=path.join(here,'render-frames-v8'),outputDir=path.resolve(here,'../../output2');
...
const output=path.join(outputDir,`s06-s07-execution-exit-v${Math.max(7,...versions)+1}.mp4`);
...
try{
  ...
  await page.goto(pathToFileURL(path.join(here,'index-v8.html')).href);
  ...
  const exit=await page.evaluate(()=>{
    ...
    const start=rect(19.95),middle=rect(20.45),end=rect(20.95),reset=rect(19.95);
...
await writeFile(path.join(here,'verification-v8','render.json'),JSON.stringify({output,frames:count,fps,errors,probe},null,2));
