...
const pan={x:0,y:0},manualPan={x:0,y:0};
const workflowCanvas=$('.workflow-canvas');
function applyPan(){
  const x=pan.x+manualPan.x,y=pan.y+manualPan.y;
  gsap.set('.workflow-pan',{x,y});workflowCanvas.style.backgroundPosition=`${x}px ${y}px`;
}
tl.to(pan,{x:-115,y:-28,duration:.8,ease:'power2.inOut'},17.65);
tl.to(pan,{x:-32,y:16,duration:.55,ease:'power2.inOut'},18.55);
...
const pointerCues=[{selector:'#send',start:5.875,click:6.491,end:6.88},{selector:'#review',start:13.38,click:13.65,end:13.72},{selector:'#confirm',start:14.79,click:15.1,end:15.35}];
...
function pointerAt(t){
  if(!interactive&&t>=17.05&&t<19.65){
    const stage=$('#stage').getBoundingClientRect(),canvas=workflowCanvas.getBoundingClientRect(),unit=1920/stage.width;
    const approach=gsap.parseEase('minaraEnter')(clamp01((t-17.05)/.5)),leave=clamp01((t-19.2)/.45);
    const x=(canvas.x-stage.x)*unit+820+pan.x+(1-approach)*100+leave*55;
    const y=(canvas.y-stage.y)*unit+210+pan.y+(1-approach)*65+leave*30;
    gsap.set('#cursor',{x:x-3,y:y-2,autoAlpha:1-leave});
    gsap.set('#ring',{autoAlpha:t>=17.55&&t<19.15?.6:0,scale:.72});
    return;
  }
...
function seek(t){manualPan.x=manualPan.y=0;driver?.kill();driver=null;playhead.time=Math.max(0,Math.min(duration(),Number(t)||0));sync(playhead.time);}
