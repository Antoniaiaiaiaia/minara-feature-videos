...
const frames=path.join(here,'render-frames-v10'),outputDir=path.resolve(here,'../../output2');
...
const output=path.join(outputDir,`s06-s07-execution-exit-v${Math.max(9,...versions)+1}.mp4`);
...
try{
  ...
  await page.goto(pathToFileURL(path.join(here,'index-v10.html')).href);
...
await writeFile(path.join(here,'verification-v10','render.json'),JSON.stringify({output,frames:count,fps,errors,probe},null,2));
