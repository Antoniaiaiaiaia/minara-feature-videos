...
const frames=path.join(here,'render-frames-v6'),outputDir=path.resolve(here,'../../output2');
...
try{
  ...
  await page.goto(pathToFileURL(path.join(here,'index-v6.html')).href);
  ...
  const exit=await page.evaluate(()=>{
    ...
    const start=rect(21.55),middle=rect(22.05),end=rect(22.55),reset=rect(21.55);
...
await writeFile(path.join(here,'verification-v6','render.json'),JSON.stringify({output,frames:count,fps,errors,probe},null,2));
