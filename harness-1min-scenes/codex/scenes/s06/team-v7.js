/* Illustrative research dialogue. User reference: references/s06-s07-team-v7.jpg.
...
const orderHome=document.createElement('div');orderHome.className='order-home';
researchOrder.before(orderHome);orderHome.append(researchOrder);
const orderSlot=document.createElement('div');orderSlot.className='research-order-slot';
document.querySelector('#research-thread').append(orderSlot);
// Keep the same live card and button. Both slots retain layout when seeking/reparenting.
orderHome.style.height=orderSlot.style.height=`${researchOrder.offsetHeight}px`;
function placeResearchOrder(t){
  const inThread=t<13.5,target=inThread?orderSlot:orderHome;
  if(researchOrder.parentElement!==target)target.append(researchOrder);
  let x=0,y=0;
  if(!inThread&&t<14.2){
    const from=orderSlot.getBoundingClientRect(),to=orderHome.getBoundingClientRect();
    const scale=document.querySelector('#stage').getBoundingClientRect().width/1920;
    const progress=gsap.parseEase('power3.inOut')(Math.max(0,Math.min(1,(t-13.5)/.7)));
    x=(from.left-to.left)/scale*(1-progress);y=(from.top-to.top)/scale*(1-progress);
  }
  gsap.set(researchOrder,{x,y});
}
function addResearchMotion(){
  ...
  tl.to('.research-heading,.research-row',{autoAlpha:0,duration:.45},13.5);
  tl.to('#research',{autoAlpha:0,duration:.2},14.05);
