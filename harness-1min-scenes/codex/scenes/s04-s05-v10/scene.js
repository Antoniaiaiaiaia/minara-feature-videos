...
const cards=assets.map((a,i)=>{
 ...
 return {a,amount,wrap,tag:wrap.querySelector('.signal-overlay'),metrics:wrap.querySelector('.metrics'),order:wrap.querySelector('.wall-order'),filled:wrap.querySelector('.wall-filled'),col:i%7,row:Math.floor(i/7),signal:i%5!==4,trade:i%5<3,cue,orderCue:cue+.46+((i*3)%5)*.07};
});
function paintWallOrder(c,t,on){
 const placed=on&&c.trade?beat(t,c.orderCue,.22,'back.out(1.8)'):0,reveal=clamp(placed);
 ...
 c.metrics.style.opacity=1-reveal;c.order.style.opacity=reveal;c.order.style.transform=`translateY(${6*(1-reveal)}px)`;
