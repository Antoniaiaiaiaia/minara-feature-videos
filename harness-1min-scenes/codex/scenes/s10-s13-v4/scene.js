...
function paint(t){
 ...
 const active=state.running.filter(Boolean).length;$('#portfolio-summary').innerHTML=`Running ${active} <span>·</span> Paused ${3-active}`;
...
timeline.set('#navigation,#market,#details,#portfolio,#cursor',{autoAlpha:0},0)
 ...
 .fromTo('#details',{scale:.89,y:42,autoAlpha:0},{scale:1,y:0,autoAlpha:1,duration:.52},5.05);
...
timeline.to('#run',{scale:.97,duration:.07},RUN).to('#run',{scale:1,duration:.22},RUN+.07)
 .to('#details',{autoAlpha:0,scale:.96,duration:.32},PORTFOLIO)
...
timeline.set('#cursor',{autoAlpha:1,x:1380,y:470},2.5)
 ...
 .to('#cursor',{x:432,y:572,duration:.45,ease:'power2.inOut'},4.50)
 .to('#cursor',{x:310,y:601,duration:.42,ease:'power2.inOut'},5.25)
 .to('#cursor',{x:800,y:493,duration:.36,ease:'power2.inOut'},5.88)
 .to('#cursor',{x:1460,y:535,duration:.36,ease:'power2.inOut'},6.5)
 .to('#cursor',{x:1482,y:927,duration:.45,ease:'power2.inOut'},7.22)
 .to('#cursor',{x:1195,y:131,duration:.58,ease:'power2.inOut'},8.42)
 .to('#cursor',{x:580,y:1000,duration:.55,ease:'power2.inOut'},9.18)
 .to('#cursor',{x:432,y:662,duration:.44,ease:'power2.inOut'},11.34)
 .to('#cursor',{x:960,y:662,duration:.47,ease:'power2.inOut'},12.03)
...
function manualView(view,reset=false){
 ...
 for(const name of ['market','details','portfolio'])gsap.set('#'+name,{autoAlpha:name===view?1:0,x:0,y:0,scale:1});
...
addEventListener('resize',fit);fit();window.scene={seek,play,pause:stop,manualView,state,timeline,duration:DURATION,catalogue};window.__timelines={'s10-s13-v4':timeline};seek(0);document.fonts.ready.then(()=>seek(0));
