// Reuses the project browser PNG → FFmpeg pipeline. User approved v6 render.
...
const output=path.resolve(here,'../../output2/s03-product-reveal-v6.mp4');
...
try{
 ...
 await page.goto(pathToFileURL(path.join(here,'_test-v6.html')).href);
...
const picks=[5,15,30,45,60,70,81,89].map(f=>`eq(n,${f})`).join('+');
