...
video = scene.parents[1] / 'output/s01-introducing-minara-harness-v6.mp4'
...
assert ink(60) < ink(59) / 3
assert ink(120) < ink(119) / 3
assert ink(179) > 1000
subprocess.run([
    'ffmpeg', '-v', 'error', '-y', '-i', str(video), '-vf',
    'select=' + '+'.join(f'eq(n\\,{f})' for f in [0, 30, 59, 60, 90, 119, 120, 149, 179])
    + ',scale=533:300,tile=3x3', '-frames:v', '1', str(scene / 'verification-v6.jpg')
], check=True)
print('PASS: 180 silent 1080p/30fps frames; exactly six seconds; separate cards cut at 2s and 4s.')
