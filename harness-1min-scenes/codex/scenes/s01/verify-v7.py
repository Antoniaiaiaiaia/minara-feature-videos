...
video = scene.parents[1] / 'output2/s01-introducing-minara-harness-v7.mp4'
...
assert ink(120) < ink(119) / 3
assert ink(179) > 1000
subprocess.run([
    'ffmpeg', '-v', 'error', '-y', '-i', str(video), '-vf',
    'select=' + '+'.join(f'eq(n\\,{f})' for f in [0, 18, 29, 30, 48, 65, 119, 120, 179])
    + ',scale=533:300,tile=3x3', '-frames:v', '1', str(scene.parents[1] / 'output2/s01-verification-v7.jpg')
], check=True)
print('PASS: 180 silent 1080p/30fps frames; exactly six seconds; separate cards cut at 1s and 4s.')
