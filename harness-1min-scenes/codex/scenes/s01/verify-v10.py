...
video = scene.parents[1] / 'output2/s01-introducing-minara-harness-v10.mp4'
...
assert ink(90) < ink(89) / 3
assert ink(179) > 1000
subprocess.run([
    'ffmpeg', '-v', 'error', '-y', '-i', str(video), '-vf',
    'select=' + '+'.join(f'eq(n\\,{f})' for f in [0, 29, 30, 50, 74, 89, 90, 119, 179])
    + ',scale=533:300,tile=3x3', '-frames:v', '1', str(scene.parents[1] / 'output2/s01-verification-v10.jpg')
], check=True)
print('PASS: 180 silent 1080p/30fps frames; exactly six seconds; separate cards cut at 1s and 3s.')
